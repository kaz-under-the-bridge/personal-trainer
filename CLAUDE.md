...
4. **週間レポート更新**: 今週の週間レポート（`reports/weekly/YYYY-QN/`）にデータを追記
   - 週の最終日（金曜日）であれば、週間総括レポートを生成する
   - `python3 scripts/generate_weekly_reports.py` で再生成してもよい
5. **食事フィードバック**: ユーザーから食事変更の報告があれば `python3 scripts/calc.py` で計算
6. **レポート出力**: `docs/system_prompt.md` のレポート構造に従って回答
7. **PR/merge**: データ追記とレポート更新をまとめてPR作成→マージする
